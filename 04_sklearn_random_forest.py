# Template to perform random fores classifier and regression with Scikit-learn and pandas 
# Parameters are definied in "Define hyperparameters and models, and applicable datasets"
import matplotlib.pyplot as plt
import pandas as pd
import sklearn_helper

def sklearn_random_forest(x_pd_dataframe, y_pd_series, mode = 'cls', max_depth = 'None', n_estimators=10, plot_feature_importance = 'y'):
    ...
    if mode == 'reg':
        ml_model = RandomForestRegressor(n_estimators = n_estimators, max_depth=max_depth, random_state=random_state_num)
    elif mode == 'cls':
        ml_model = RandomForestClassifier(n_estimators = n_estimators, max_depth=max_depth, random_state=random_state_num)
    else:
...
def main():
    ...
    sklearn_random_forest(x_pd_dataframe, y_pd_series, mode = mode, max_depth = num_max_depth, n_estimators = num_estimator)
